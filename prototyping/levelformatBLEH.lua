...
axiom.LevelChunk = concept{
    formatmagic = "LVL",
    formatversion = 9,
}

local function cdefstruct(name, fieldstr, dontpack, meta)
    local ctcdef = "struct "..(dontpack and "" or "__attribute__((__packed__)) ").."{ "..fieldstr.." } "
    if meta then
        axiom.LevelChunk["ct_"..name] = ffi.metatype(ctcdef, meta)
    else
        axiom.LevelChunk["ct_"..name] = ffi.typeof(ctcdef)
    end
end
...
function axiom.LevelChunk.__init(class, existingObj, ...)
...
do -- decode
    do -- iterateChunks
        ...
        function axiom.iterateChunks(reader, maxsize)
            local maxoffset
            if maxsize then
            ...
            return function()
                ...
                local chunk_header, _, chunk_header_data = readstructptr("chunk_header", reader, axiom.ct_chunkheader)
    ...
    do -- decode
        ...
        function axiom.decode(reader)
            local levelchunk = axiom.LevelChunk()
            
            local endpos = assert(reader:seek("end"))
            assert(reader:seek("set", 0))
            
            local level_header, _, level_header_data = readstructptr("level_header", reader, axiom.ct_fileheader)
            assert(ffi.C.memcmp(level_header.magic, axiom.formatmagic, 3) == 0, "not a NS2 level")
            assert(level_header.version == axiom.formatversion, "unsupported level version")
            levelchunk.version = level_header.version
            local chunk_i, dbglastparsedid = 1, -1
            for chunk_header, chunk_header_data in axiom.iterateChunks(reader, endpos-reader:seek()) do
                local startpos = reader:seek()
                
                local idstr = chunkidenum[chunk_header.id] or chunk_header.id
                --local parsefunc = axiom["parseChunk_"..idstr] or error(("unknown chunk id (%s)"):format(chunk_header.id))
                local parsefunc = axiom["parseChunk_"..idstr] or axiom.parseChunk_Unknown
...
do -- encode
    do
        --local DBG,writeprimitive,writestring=true,writeprimitive_dbg,writestring_dbg
        function axiom.encode(levelchunk)
            local size = 0
            local wt = {dbgsize = 0}
            
            assert(levelchunk.version == 9, "unsupported level version")
            size = size+writestring("level_header_magic", wt, axiom.formatmagic, false, true)
            size = size+writeprimitive("level_header_version", wt, levelchunk.formatversion, "uint8_t")
            
            local chunklistmap = levelchunk.chunklistmap
            -- Is the order of chunks important?
            local chunknamesinorder = { "Mesh", "Object", "Layers", "Groups" }
            for chunknamenum, chunkname in ipairs(chunknamesinorder) do
                size = size+writechunklist(wt,
                    chunklistmap[chunkname] or {},
                    chunkidenum[chunkname],
                    levelchunk["writeChunk_"..chunkname],
                    DBG and "----- Chunk_"..chunkname
                )
            end
            ...
            for chunkidstr, chunklist in pairs(chunklistmap) do
                if not chunkidenum[chunkidstr] then
                    ...
                    size = size+writechunklist(wt, chunklist, chunkid, levelchunk.writeChunk_Unknown, DBG and "----- Chunk_"..chunkidstr)
                end
            end
...
do -- Chunk_Groups
    ...
    function axiom.LevelChunk.parseChunk_Groups(reader, chunk_header)
        local chunk = {}
        ...
        for group_i = 1, grouplist_length do
            ...
            group.color = readstruct("group_color", reader, axiom.LevelChunk.ct_col4)
    ...
    --local DBG,writeprimitive,writestring=true,writeprimitive_dbg,writestring_dbg
    function axiom.LevelChunk.writeChunk_Groups(wt, chunk)
        local size = 0
        ...
        for group_i = 1, grouplist_length do
            ...
            size = size+writeprimitive("group_color", wt, group.color, axiom.LevelChunk.ct_col4)
...
do -- Chunk_Layers
    --local DBG,readprimitive,readstring,readstruct,readstructptr = true,readprimitive_dbg,readstring_dbg,readstruct_dbg,readstructptr_dbg
    function axiom.LevelChunk.parseChunk_Layers(reader, chunk_header)
        local chunk = {}
        ...
        for layer_i = 1, layerlist_length do
            ...
            layer.color = readstruct("layer_color", reader, axiom.LevelChunk.ct_col4)
    ...
    --local DBG,writeprimitive,writestring=true,writeprimitive_dbg,writestring_dbg
    function axiom.LevelChunk.writeChunk_Layers(wt, chunk)
        local size = 0
        ...
        for layer_i = 1, layerlist_length do
            ...
            size = size+writeprimitive("layer_color", wt, layer.color, axiom.LevelChunk.ct_col4)
...
do -- Chunk_Mesh
    --local DBG,readprimitive,readstring,readstruct,readstructptr = true,readprimitive_dbg,readstring_dbg,readstruct_dbg,readstructptr_dbg
    function axiom.LevelChunk.parseChunk_Mesh(reader, chunk_header)
        local chunk = {
            chunklistmap = {}
        }
        
        local maxoffset = reader:seek()+chunk_header.size
        for subchunk_header, subchunk_header_data in axiom.LevelChunk.iterateChunks(reader, maxoffset-reader:seek()) do
            local startpos = reader:seek()
            
            local idstr = meshsubchunkidenum[subchunk_header.id] or subchunk_header.id
            --local parsefunc = axiom.LevelChunk["parseChunk_"..idstr] or error(("unsupported mesh subchunk id (%s)"):format(subchunk_header.id))
            local parsefunc = axiom.LevelChunk["parseChunk_"..idstr] or axiom.LevelChunk.parseChunk_Unknown
    ...
    --local DBG,writeprimitive,writestring=true,writeprimitive_dbg,writestring_dbg
    function axiom.LevelChunk.writeChunk_Mesh(wt, chunk)
        local size = 0
        
        local chunklistmap, idenum = chunk.chunklistmap, meshsubchunkidenum
        local chunknamesinorder = { "Materials", "Vertices", "Edges", "Faces", "FaceLayers", "MappingGroups", "GeometryGroups", "Triangles" }
        for chunknamenum, chunkname in ipairs(chunknamesinorder) do
            size = size+writechunklist(wt,
                chunklistmap[chunkname] or {},
                idenum[chunkname],
                axiom.LevelChunk["writeChunk_"..chunkname],
                DBG and "=== Chunk_Mesh.Chunk_"..chunkname
            )
        end
        for chunkidstr, chunklist in pairs(chunklistmap) do
            if not meshsubchunkidenum[chunkidstr] and chunkidstr ~= 2 then
                ...
                writechunklist(wt, chunklist, chunkid, axiom.LevelChunk.writeChunk_Unknown, DBG and "=== Chunk_Mesh.Chunk_"..chunkidstr)
            end
        end
        return size
    end
...
do -- Chunk_Triangles
    --local DBG,readprimitive,readstring,readstruct,readstructptr = true,readprimitive_dbg,readstring_dbg,readstruct_dbg,readstructptr_dbg
    function axiom.LevelChunk.parseChunk_Triangles(reader, chunk_header)
        local chunk = {}
    ...
    --local DBG,writeprimitive,writestring=true,writeprimitive_dbg,writestring_dbg
    function axiom.LevelChunk.writeChunk_Triangles(wt, chunk)
        local size = 0
        ...
        for ghostvertex_i = 1, ghostvertexlist_length do
            size = size+writeprimitive("ghostvertexlist_ghostvertex", wt, chunk.ghostvertexlist[ghostvertex_i], axiom.LevelChunk.ct_vec3)
        end
        ...
        for smoothednormal_i = 1, smoothednormallist_length do
            size = size+writeprimitive("smoothednormallist_smoothednormal", wt, chunk.smoothednormallist[smoothednormal_i], axiom.LevelChunk.ct_vec3)
        end
...
do -- Chunk_GeometryGroups
    ...
    function axiom.LevelChunk.parseChunk_GeometryGroups(reader, chunk_header)
        local chunk = {}
    ...
    function axiom.LevelChunk.writeChunk_GeometryGroups(wt, chunk)
        local size = 0
...
do -- Chunk_MappingGroups
    ...
    function axiom.LevelChunk.parseChunk_MappingGroups(reader, chunk_header)
        local chunk = {}
        ...
        for mappinggroup_i = 1, mappinggrouplist_length do
            mappinggrouplist[mappinggroup_i] = readstruct("mappinggrouplist_mappinggroup", reader, axiom.LevelChunk.ct_mappinggroupdata)
        end
    ...
    --local DBG,writeprimitive,writestring=true,writeprimitive_dbg,writestring_dbg
    function axiom.LevelChunk.writeChunk_MappingGroups(wt, chunk)
        local size = 0
        ...
        for mappinggroup_i = 1, mappinggrouplist_length do
            size = size+writeprimitive("mappinggrouplist_mappinggroup", wt, mappinggrouplist[mappinggroup_i], axiom.LevelChunk.ct_mappinggroupdata)
        end
...
do -- Chunk_FaceLayers
    --local DBG,readprimitive,readstring,readstruct,readstructptr = true,readprimitive_dbg,readstring_dbg,readstruct_dbg,readstructptr_dbg
    function axiom.LevelChunk.parseChunk_FaceLayers(reader, chunk_header)
        local chunk = {}
...
<viewports>
  <viewport>
    ...
    <camera_yaw>3.9269</camera_yaw>
    <camera_pitch>0.45</camera_pitch>
  </viewport>
  <viewport>
